   ...
   for(int i=0;i<ntps[k]->GetEntries();i++)
    {
     ...
     if(k==0)
      _BDT_VBF_Hbb->Fill(reader->EvaluateMVA("TMVAClassification_BDTG"),S_weight[k]);
     if(k==1)
      _BDT_ggF_Hbb->Fill(reader->EvaluateMVA("TMVAClassification_BDTG"),S_weight[k]);
    }
   ...
   for(int i=0;i<ntpb[k]->GetEntries();i++)
    {
     ...
     if(k<7)
      ...
      _BDT_Single_Top->Fill(reader->EvaluateMVA("TMVAClassification_BDTG"),B_weight[k]);
     else if(k>=11 && k<14)
      _BDT_tt->Fill(reader->EvaluateMVA("TMVAClassification_BDTG"),B_weight[k]);
     else if(k>=14 && k<17)
      _BDT_WJets->Fill(reader->EvaluateMVA("TMVAClassification_BDTG"),B_weight[k]);
     else if(k>=17 && k<20)
      _BDT_ZJets->Fill(reader->EvaluateMVA("TMVAClassification_BDTG"),B_weight[k]);
     else if(k==20)
