...
// ********************** PT-Mass-SFs **************************
  TFile* f_PT_Mass_SF = new TFile("/afs/cern.ch/user/t/tumasyan/public/Corrections/Trigger_SFs_Roots/PT_Mass_2dSF_TTbar.root");
  TH2D * _Eff_Data    = (TH2D*)f_PT_Mass_SF->Get("Eff_Data_PostEE");
  TH2D * _Eff_MC      = (TH2D*)f_PT_Mass_SF->Get("Eff_MC_PostEE");
// ********************** BTG-SFs Legacy **************************
  TFile* f_BTG_SF_Legacy = new TFile("/afs/cern.ch/user/t/tumasyan/public/BTG_SFs_TXbb/TTbar/2022/BTG_SF_TXbb_PostEE_TTbar.root");
...
vPar_AK4.push_back(JetCorrectorParameters("/afs/cern.ch/user/t/tumasyan/public/2022/Summer22EE_22Sep2023_V2_MC/Summer22EE_22Sep2023_V2_MC_L2Relative_AK4PFPuppi.txt"));
FactorizedJetCorrector*  corrector_AK4 = new FactorizedJetCorrector(vPar_AK4);

vector<JetCorrectorParameters> vPar_AK8;
vPar_AK8.push_back(JetCorrectorParameters("/afs/cern.ch/user/t/tumasyan/public/2022/Summer22EE_22Sep2023_V2_MC/Summer22EE_22Sep2023_V2_MC_L2Relative_AK8PFPuppi.txt"));
...
TFile *f = new TFile("Trees_WtoLNu_4Jets.root","RECREATE");
...
// ********************************************************** Ordering FatJets by Xbb-score
    ...
    for(int ii=0;ii<3; ii++)
      if(fatJet_pt[ii] > 250 && fatJet_particleNet_Xbb_Legacy[ii] > maxXbb_1)
       {
    ...
    for(int ii=0;ii<3; ii++)
      if(fatJet_pt[ii] > 250 && ii!=Idx_1 && fatJet_particleNet_Xbb_Legacy[ii] > maxXbb_2)
       {
    ...
    for(int ii=0;ii<3; ii++)
      if(fatJet_pt[ii] > 250 && ii!=Idx_1 && ii!=Idx_2)
...
// ********************************************************** Trigger Objects and Matchings
  ...
  bool matched_to_AK8PFJet250_SoftDropMass40_ParticleNetBB0p35_1 = false;
  for(int itrg=0;itrg<NTrigger_Objects;itrg++)
    if((Trigger_Object_bit[itrg] & 12) == 12)
       if(sqrt(pow((fatJet_eta[Idx_1] - Trigger_Object_eta[itrg]),2) + pow(phi_dist(fatJet_phi[Idx_1],Trigger_Object_phi[itrg]),2)) < 0.4 && Trigger_Object_pt[itrg] > 100) 
         {matched_to_AK8PFJet250_SoftDropMass40_ParticleNetBB0p35_1 = true; break;}
  if (matched_to_AK8PFJet250_SoftDropMass40_ParticleNetBB0p35_1) matched_TRG_1=true;

  // Matching 2nd
  bool matched_TRG_2=false;
  bool matched_to_AK8PFJet250_SoftDropMass40_ParticleNetBB0p35_2 = false;
  for(int itrg=0;itrg<NTrigger_Objects;itrg++)
    if((Trigger_Object_bit[itrg] & 12) == 12)
       if(sqrt(pow((fatJet_eta[Idx_2] - Trigger_Object_eta[itrg]),2) + pow(phi_dist(fatJet_phi[Idx_2],Trigger_Object_phi[itrg]),2)) < 0.4 && Trigger_Object_pt[itrg] > 100) 
         {matched_to_AK8PFJet250_SoftDropMass40_ParticleNetBB0p35_2 = true; break;}
  if (matched_to_AK8PFJet250_SoftDropMass40_ParticleNetBB0p35_2) matched_TRG_2=true;
...
// ********************************************************** weight
   ...
   if(matched_to_AK8PFJet250_SoftDropMass40_ParticleNetBB0p35_1)
     {
   ...
   if(matched_to_AK8PFJet250_SoftDropMass40_ParticleNetBB0p35_2)
     {
