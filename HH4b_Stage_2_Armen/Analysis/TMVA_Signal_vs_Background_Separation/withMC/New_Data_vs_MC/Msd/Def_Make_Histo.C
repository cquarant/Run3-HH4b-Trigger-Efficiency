...
//  XTitle = "FJet_{1} - #tau_{3} / #tau_{2}";
  ...
  Float_t Bound[9] = {0,50,66,83,100,150,182,250,500};
  TH1D *_My_Data     = new TH1D("My_Data","My_Data",8,Bound);
  TH1D *_My_qcd       = new TH1D("My_qcd","My_qcd",8,Bound);
  TH1D *_My_tt       = new TH1D("My_tt","My_tt",8,Bound);
  TH1D *_My_vj       = new TH1D("My_vj","My_vj",8,Bound);
  TH1D *_My_vv       = new TH1D("My_vv","My_vv",8,Bound);
...
//  TFile* f_TTbar  = new TFile("../MC/Histograms_TTbar.root");
  TFile* f_TTbar  = new TFile("../MC/Histograms_TTbar_All_Corr.root");
...
/*
  _Data_var->Scale(968.0/1462.0);
  _QCD_var->Scale(512.6/736.6);
  _TTbar_var->Scale(569.0/882.0);
//  _TTbar_var->Scale(467/741.0);   // corr
  _VJ_var->Scale(17.6/24);
  _VV_var->Scale(0.2/0.28);
*/
...
// ------------>Primitives in pad: c1_1
  ...
  TString YTitle_10 = "Events";
   ...
   for(int i=1; i<=_My_Data->GetSize();i++)
     {
      _My_Data->SetBinContent(i,0.0);
      _My_qcd->SetBinContent(i,0.0);
      _My_tt->SetBinContent(i,0.0);
      _My_vj->SetBinContent(i,0.0);
      _My_vv->SetBinContent(i,0.0);
     }

   for(int i=1; i<=_TTbar_var->GetSize()-2; i++)
     {
        Int_t bin  = _My_Data->GetXaxis()->FindBin(i - 0.5);
        double current_val_Data = _My_Data->GetBinContent(bin);
        double current_val_qcd = _My_qcd->GetBinContent(bin);
        double current_val_tt = _My_tt->GetBinContent(bin);
        double current_val_vj = _My_vj->GetBinContent(bin);
        double current_val_vv = _My_vv->GetBinContent(bin);
        current_val_Data = current_val_Data + _Data_var->GetBinContent(i);
        current_val_qcd = current_val_qcd + _QCD_var->GetBinContent(i);
        current_val_tt = current_val_tt + _TTbar_var->GetBinContent(i);
        current_val_vj = current_val_vj + _VJ_var->GetBinContent(i);
        current_val_vv = current_val_vv + _VV_var->GetBinContent(i);
       _My_Data->SetBinContent(bin, current_val_Data);
       _My_qcd->SetBinContent(bin, current_val_qcd);
       _My_tt->SetBinContent(bin, current_val_tt);
       _My_vj->SetBinContent(bin, current_val_vj);
       _My_vv->SetBinContent(bin, current_val_vv);
      }


   _My_Data->SetBinContent(5, 0);
   _My_qcd->SetBinContent(5, 0);
   _My_tt->SetBinContent(5, 0);
   _My_vj->SetBinContent(5, 0);
   _My_vv->SetBinContent(5, 0);
...
//  _QCD_var->Scale(Offline_BTG_SF);
  ...
  double ymin=10e-3;
  double ymax=3*(_QCD_var->GetMaximum());
