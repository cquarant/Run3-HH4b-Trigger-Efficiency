...
void tree_data_hl(const std::string &lepton,       // Example: Run3
...
#include "/afs/cern.ch/work/c/cquarant/Hbt2/CMSSW_16_0_0_pre3/src/TTbarBkgEstimation/Run3-HH4b-Trigger-Efficiency/GoodLumiList/GoodLumiList_Map_2023D.txt"
  ...
  for (int i = 0; i < 4; i++) {
    ...
    InputTree->SetBranchAddress(("ak4JetPt" + idx).c_str(), &ak4JetPt[i]);
    InputTree->SetBranchAddress(("ak4JetEta" + idx).c_str(), &ak4JetEta[i]);
    InputTree->SetBranchAddress(("ak4JetPhi" + idx).c_str(), &ak4JetPhi[i]);
    InputTree->SetBranchAddress(("ak4JetMass" + idx).c_str(), &ak4JetMass[i]);
    InputTree->SetBranchAddress(("ak4JetbtagPNetB" + idx).c_str(), &ak4JetbtagPNetB[i]);
  } 
  ...
  for (int i = 0; i < InputTree->GetEntries(); i++) {
    ...
    // Select the best ak4 jet candidate based on b-tag, pt, and dR to lepton and fatjet
    ...
    // if ((ak4JetPt0 > 40 && ak4JetbtagPNetB0 > 0.90) || (ak4JetPt1 > 40 && ak4JetbtagPNetB1 > 0.90)) {
    //   continue;
    // }
    // h_cutflow->Fill(7.5); // After ak4 jet b-tag and pt selection
    ...
    // if (ak4JetPt0 > 40 && ak4JetbtagPNetB0 > 0.90) {
    //   dR_J1FJ = get_dR(ak4JetEta0, ak4JetPhi0, ak8FatJetEta0, ak8FatJetPhi0);
    //   dR_J1L = get_dR(ak4JetEta0, ak4JetPhi0, LeptonEta0, LeptonPhi0);
    // }
    // if (ak4JetPt1 > 40 && ak4JetbtagPNetB1 > 0.90) {
    //   dR_J2FJ = get_dR(ak4JetEta1, ak4JetPhi1, ak8FatJetEta0, ak8FatJetPhi0);
    //   dR_J2L = get_dR(ak4JetEta1, ak4JetPhi1, LeptonEta0, LeptonPhi0);
    // }

    // dR_JFJ_min = min(dR_J1FJ, dR_J2FJ);
    // dR_JFJ_max = max(dR_J1FJ, dR_J2FJ);
    // dR_JmaxL = max(dR_J1L, dR_J2L);

    // if ((dR_J1L > 0. && dR_J1L <= 0.4) || (dR_J2L > 0. && dR_J2L <= 0.4)) {
    //   continue;
    // }

    // h_cutflow->Fill(7.5); // Requiring lepton not to be close to selected ak4 jets

    // if ( (dR_J1FJ > 0. && dR_J1FJ <= 1.5) || (dR_J2FJ > 0. && dR_J2FJ <= 1.5)) {
    //   continue;
    // }
    // h_cutflow->Fill(8.5); // Requiring ak4 jets not to be close to fatjet
    ...
    T_jet1_pt = ak4JetPt[id_selected];
    T_jet1_eta = ak4JetEta[id_selected];
    T_jet1_phi = ak4JetPhi[id_selected];
    T_jet1_mass = ak4JetMass[id_selected];
    T_jet1_btag = ak4JetbtagPNetB[id_selected];
    // T_jet2_pt = ak4JetPt1;
