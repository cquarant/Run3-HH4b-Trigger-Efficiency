...
void plot(const std::string &year,         // 2022, 2023
  ...
  } else if (variable.Contains("GloParT") && variable.Contains("Mass")) {
    std::vector<Double_t> bins;
    for (int i = MASS_MIN; i <= MASS_MAX; i += MASS_STEP) {
      bins.push_back(i);
    }
  ...
  for (int iB = 1; iB <= _TTbar_var->GetNbinsX(); ++iB) {
    ...
    if (has_TTbar_syst) {
      float eSystUp = _TTbar_var_up->GetBinContent(iB) - _TTbar_var->GetBinContent(iB);
      float eSystDown = _TTbar_var_down->GetBinContent(iB) - _TTbar_var->GetBinContent(iB);
      float eSyst = TMath::Max(TMath::Abs(eSystUp), TMath::Abs(eSystDown));
      Err = TMath::Sqrt(Err * Err + eSyst * eSyst);
