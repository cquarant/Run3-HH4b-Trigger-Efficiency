...
//  variable = "MET";
  ...
  TFile* f_QCD      = new TFile("../MC/Histograms_QCD.root");
  TFile* f_QCD_SF   = new TFile("../MC/Histograms_QCD.root");
  TFile* f_VV       = new TFile("../MC/Histograms_VV.root");
  TFile* f_VV_SF    = new TFile("../MC/Histograms_VV.root");
  TFile* f_VJ       = new TFile("../MC/Histograms_VJ.root");
  TFile* f_VJ_SF    = new TFile("../MC/Histograms_VJ.root");

  TFile* f_TTbar    = new TFile("../MC/Histograms_TTbar.root");
  TFile* f_TTbar_SF = new TFile("../MC/Histograms_TTbar_All_Corr.root");
  ...
  TH1D * _VJ = (TH1D*)f_VJ->Get(variable);
  TH1D * _VJ_SF = (TH1D*)f_VJ_SF->Get(variable);
...
/*
  _Data_var_10->Scale(968.0/1462.0);
//  _Data_var_11->Scale(968.0/1462.0);
  _QCD->Scale(512.6/736.6);
  _QCD_SF->Scale(512.6/736.6);
  _VJ->Scale(17.6/24);
  _VJ_SF->Scale(17.6/24);
  _VV->Scale(0.2/0.28);
  _VV_SF->Scale(0.2/0.28);

  _TTbar->Scale(569.0/882.0);
  _TTbar_SF->Scale(467/741.0);
*/

  for (int iB=1; iB<=_Data_var_10->GetSize(); ++iB)
     {
       _Data_var_10->SetBinContent(iB,(int)(_Data_var_10->GetBinContent(iB)+0.5));
       _Data_var_10->SetBinError(iB,sqrt(_Data_var_10->GetBinContent(iB)));
     }


/*
  for (int iB=1; iB<=_Data_var_10->GetSize(); ++iB)
    if( _QCD_SF->GetBinContent(iB) > 2.0 * _Data_var_10->GetBinContent(iB))
     {
         _QCD->SetBinContent(iB, _QCD->GetBinContent(iB)/1.2);
         _QCD->SetBinError(iB, _QCD->GetBinError(iB)/1.2);
         _QCD_SF->SetBinContent(iB, _QCD_SF->GetBinContent(iB)/1.2);
         _QCD_SF->SetBinError(iB, _QCD_SF->GetBinError(iB)/1.2);
     }
*/
/*
  for (int iB=1; iB<=_Data_var_10->GetSize(); ++iB) {
       if(iB<95 || iB>155)
        {
         _QCD->SetBinContent(iB,0);
         _QCD_SF->SetBinError(iB,0);
         _QCD_SF->SetBinContent(iB,0);
         _TTbar->SetBinContent(iB,0);
         _TTbar_SF->SetBinContent(iB,0);
         _VV->SetBinContent(iB,0);
         _VV_SF->SetBinContent(iB,0);
         _VJ->SetBinContent(iB,0);
         _VJ_SF->SetBinContent(iB,0);
         _Data_var_10->SetBinContent(iB,0);
         _Data_var_11->SetBinContent(iB,0);
        }
    }
*/
// ****************************************************************
...
//  _QCD_SF->Scale(1);
  ...
  _VJ_SF->SetFillColor(kGreen-3);
  _VJ_SF->SetLineWidth(1);
  _VJ_SF->SetLineStyle(1);
  _VJ_SF->SetMaximum(ymax);
  _VJ_SF->SetMinimum(ymin);
  _VJ_SF->Draw("HIST same");
...
// ******************************* Legend
    ...
    t1->AddText("#it{                                        CMS (2022 C-G) preliminary ~ 35 fb^{-1}}");
    t1->Draw("same");
...
//    t11->Draw("same");
    ...
    t2->AddText("#it{                                        CMS (2022 C-G) preliminary ~ 35 fb^{-1}}");
    t2->Draw("same");
