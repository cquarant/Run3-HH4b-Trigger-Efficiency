...
def calculate_kfactor(year, ttbar_corr="", key="pTjj"):
    ...
    TTbar_label = "TTbar"
    if ttbar_corr:
        TTbar_label = f"{TTbar_label}_{ttbar_corr}"
    MC_regions = ["QCD", TTbar_label, "VJ", "VV", "ttHto2B"]
    paths_MC = {
        region: Path(f"hists/Histograms_{year}_MC_{region}.root")
        for region in MC_regions
    }
    print(f"MC paths: {paths_MC}")
    ...
    if f"{key};1" in data_file.keys():
        data_hist = data_file[f"{key};1"]
        key = f"{key};1"
    elif f"{key}_nominal;1" in data_file.keys():
        data_hist = data_file[f"{key}_nominal;1"]
        key = f"{key}_nominal;1"
    else:
        raise KeyError(f"Keys: {data_file.keys()}")
...
def main():
    ...
    parser.add_argument("--ttbar-corr", type=str, default="", help='Correction flag to ttbar')
    args = parser.parse_args()
    
    try:
        kfactor = calculate_kfactor(args.year, ttbar_corr=args.ttbar_corr)
        update_kfactor_file(args.year, kfactor)
        print(f"Successfully calculated and updated k-factor for {args.year}: {kfactor}")
