...
// ********************** BTG-SFs **************************
  TFile* f_BTG_SF       = new TFile("/afs/cern.ch/work/t/tumasyan/HHTo4B/2023/CMSSW_13_1_0/src/HHBoostedAnalyzer/MyHisto/NanoAOD_V12/MC/Signal/Legacy_Test/Legacy_results/BTG_SF_Def.root");
  TH1D * _BTG_Eff_Data  = (TH1D*)f_BTG_SF->Get("BTG_Eff_Data");
  TH1D * _BTG_Eff_MC    = (TH1D*)f_BTG_SF->Get("BTG_Eff_MC");
// ********************** BTG-SFs Legacy **************************
  TFile* f_BTG_SF_Legacy = new TFile("/afs/cern.ch/work/t/tumasyan/HHTo4B/2023/CMSSW_13_1_0/src/HHBoostedAnalyzer/MyHisto/NanoAOD_V12/MC/Signal/Legacy_Test/Legacy_results/BTG_SF_Legacy.root");
...
void Making_Tree_QCD_HT_400to600()
{
...
// *********************************************************
*/ TFile *f = new TFile("Histograms_QCD_HT_400to600.root","RECREATE");
...
// ********************************************************** Ordering FatJets by Xbb-score
    ...
    for(int ii=0;ii<3; ii++)
      if(fatJet_pt[ii] > 230 && fatJet_particleNet_XbbVsQCD[ii] > maxXbb_1)
       {
         maxXbb_1 = fatJet_particleNet_XbbVsQCD[ii];
         Idx_1 =ii; // 1st Xbb tagged
       }

    for(int ii=0;ii<3; ii++)
      if(fatJet_pt[ii] > 230 && ii!=Idx_1 && fatJet_particleNet_XbbVsQCD[ii] > maxXbb_2)
       {
         maxXbb_2 = fatJet_particleNet_XbbVsQCD[ii];
    ...
    if(fatJet_pt[Idx_1] < 240 || fabs(fatJet_eta[Idx_1]) > 2.5 || fatJet_msoftdrop[Idx_1] < 50) continue;
    if(fatJet_pt[Idx_2] < 240 || fabs(fatJet_eta[Idx_2]) > 2.5 || fatJet_msoftdrop[Idx_2] < 50) continue;

    if(fatJet_particleNet_XbbVsQCD[Idx_1] < 0.8) continue;
...
// ********************************************************** weight
   ...
   weight = (weight/SumGenWeights)*XSec_QCD_HT_400to600*Lumi;
   ...
   double Eff_Data_1 = 0;
   double Eff_MC_1   = 0;
   double Eff_Data_BTG_1 = 0;
   double Eff_MC_BTG_1   = 0;

   double Eff_Data_1_Legacy = 0;
   double Eff_MC_1_Legacy   = 0;
   ...
   if(matched_to_AK8PFJet230_SoftDropMass40_1)
     {
      ...
      Eff_Data_1_Legacy = Eff_Data_1;
      Eff_MC_1_Legacy   = Eff_MC_1;

      Int_t bin_BTG_1 = _BTG_Eff_Data->GetXaxis()->FindBin(fatJet_particleNet_XbbVsQCD[Idx_1]);
      Eff_Data_BTG_1  = _BTG_Eff_Data->GetBinContent(bin_BTG_1);
      Eff_MC_BTG_1    = _BTG_Eff_MC->GetBinContent(bin_BTG_1);
      ...
      Eff_Data_1 = Eff_Data_1 * Eff_Data_BTG_1;
      Eff_MC_1 = Eff_MC_1 * Eff_MC_BTG_1;
      Eff_Data_1_Legacy = Eff_Data_1_Legacy * Eff_Data_BTG_1_Legacy;
      Eff_MC_1_Legacy = Eff_MC_1_Legacy * Eff_MC_BTG_1_Legacy;

      if(Eff_Data_BTG_1 <=0 || Eff_MC_BTG_1<=0 || Eff_Data_BTG_1_Legacy<=0 || Eff_MC_BTG_1_Legacy<=0)
      cout<<fatJet_pt[Idx_1]<<"       "<<fatJet_msoftdrop[Idx_1]<<"       "<<fatJet_particleNet_XbbVsQCD[Idx_1]<<"    --- Effs ---     "<<Eff_Data_1<<"     "<<Eff_Data_BTG_1<<"      "<<Eff_MC_1<<"      "<<Eff_MC_BTG_1<<endl;

     }

   double Eff_Data_2 = 0;
   double Eff_MC_2   = 0;
   double Eff_Data_BTG_2 = 0;
   double Eff_MC_BTG_2   = 0;

   double Eff_Data_2_Legacy = 0;
   double Eff_MC_2_Legacy   = 0;
   ...
   if(matched_to_AK8PFJet230_SoftDropMass40_2)
     {
      ...
      Eff_Data_2_Legacy = Eff_Data_2;
      Eff_MC_2_Legacy   = Eff_MC_2;

      Int_t bin_BTG_2 = _BTG_Eff_Data->GetXaxis()->FindBin(fatJet_particleNet_XbbVsQCD[Idx_2]);
      Eff_Data_BTG_2  = _BTG_Eff_Data->GetBinContent(bin_BTG_2);
      Eff_MC_BTG_2    = _BTG_Eff_MC->GetBinContent(bin_BTG_2);
      ...
      Eff_Data_2 = Eff_Data_2 * Eff_Data_BTG_2;
      Eff_MC_2 = Eff_MC_2 * Eff_MC_BTG_2;
      Eff_Data_2_Legacy = Eff_Data_2_Legacy * Eff_Data_BTG_2_Legacy;
      Eff_MC_2_Legacy = Eff_MC_2_Legacy * Eff_MC_BTG_2_Legacy;

      if(Eff_Data_BTG_2 <=0 || Eff_MC_BTG_2<=0 || Eff_Data_BTG_2_Legacy<=0 || Eff_MC_BTG_2_Legacy<=0)
      cout<<fatJet_pt[Idx_2]<<"       "<<fatJet_msoftdrop[Idx_2]<<"       "<<fatJet_particleNet_XbbVsQCD[Idx_2]<<"    --- Effs ---     "<<Eff_Data_2<<"     "<<Eff_Data_BTG_2<<"      "<<Eff_MC_2<<"      "<<Eff_MC_BTG_2<<endl;
     }
   ...
   cout<<Tot_Data<<"        "<<Tot_MC<<endl;
   if(PT_Mass_BTG_SF > 1.4)
    {
     cout<<"aaaaaaaaaaaaaaaaaaaaaa"<<endl;
     cout<<fatJet_pt[Idx_1]<<"       "<<fatJet_eta[Idx_1]<<"       "<<fatJet_phi[Idx_1]<<"       "<<fatJet_msoftdrop[Idx_1]<<"       "<<fatJet_particleNet_XbbVsQCD[Idx_1]<<endl;
     cout<<fatJet_pt[Idx_2]<<"       "<<fatJet_eta[Idx_2]<<"       "<<fatJet_phi[Idx_2]<<"       "<<fatJet_msoftdrop[Idx_2]<<"       "<<fatJet_particleNet_XbbVsQCD[Idx_2]<<endl;
     cout<<fatJet_pt[Idx_3]<<"       "<<fatJet_eta[Idx_3]<<"       "<<fatJet_phi[Idx_3]<<"       "<<fatJet_msoftdrop[Idx_3]<<"       "<<fatJet_particleNet_XbbVsQCD[Idx_3]<<endl<<endl;

    for(int itrg=0;itrg<NTrigger_Objects;itrg++)
//      if((Trigger_Object_bit[itrg] & 4) == 4)
       cout<<Trigger_Object_pt[itrg]<<"       "<<Trigger_Object_eta[itrg]<<"       "<<Trigger_Object_phi[itrg]<<"      "<<Trigger_Object_bit[itrg]<<endl;
    }


   cout<<endl<<endl;

   double Tot_Data_Legacy = 1 - (1 - Eff_Data_1_Legacy)*(1 - Eff_Data_2_Legacy);
   double Tot_MC_Legacy   = 1 - (1 - Eff_MC_1_Legacy)*(1 - Eff_MC_2_Legacy);
