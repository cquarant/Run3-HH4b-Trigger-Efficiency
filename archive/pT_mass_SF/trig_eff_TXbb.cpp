...
void trig_eff_TXbb(const std::string& data_path, const std::string& mc_path,
    ...
    set_histo_style_1D(_data_tag, "T_{Xbb}", "Efficiency", 0.5, 1.0);
    c1->cd();
    _data_tag->Draw("colz");
    c1->SaveAs(figure_data_path.c_str());

    set_histo_style_1D(_mc_tag, "T_{Xbb}", "Efficiency", 0.5, 1.0);
    c2->cd();
    _mc_tag->Draw("colz");
    ...
    set_histo_style_1D(_SF_TXbb, "T_{Xbb}", "Data / MC", 0.8, 1.2);
    c3->cd();
    _SF_TXbb->Draw("colz");
