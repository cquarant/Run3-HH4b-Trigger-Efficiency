...
void SF_pTjj(
  ...
  TH1D *_SF = new TH1D("SF", "SF", n_custom_bins, custom_bins);
  TH1D *hint_68 = new TH1D("hint_68", "", 370, 0.18, 1000);
  ...
  TString variable = "pTjj";
  TString XTitle = "Recoil ttbar p_{T}";
  ...
  // Create histograms with custom binning and rebin using those bins
  TH1D *_Data_var_rebin = (TH1D*)_Data_var->Rebin(n_custom_bins, "_Data_var_rebin", custom_bins);
  TH1D *_VJ_var_rebin = (TH1D*)_VJ_var->Rebin(n_custom_bins, "_VJ_var_rebin", custom_bins);
  TH1D *_VV_var_rebin = (TH1D*)_VV_var->Rebin(n_custom_bins, "_VV_var_rebin", custom_bins);
  TH1D *_QCD_var_rebin = (TH1D*)_QCD_var->Rebin(n_custom_bins, "_QCD_var_rebin", custom_bins);
  TH1D *_TTbar_var_rebin = (TH1D*)_TTbar_var->Rebin(n_custom_bins, "_TTbar_var_rebin", custom_bins);
  TH1D *_ttHto2B_var_rebin = (TH1D*)_ttHto2B_var->Rebin(n_custom_bins, "_ttHto2B_var_rebin", custom_bins);

  _TTbar_var_rebin->GetXaxis()->SetLabelFont(42);
  _TTbar_var_rebin->GetXaxis()->SetLabelOffset(0.15);
  _TTbar_var_rebin->GetXaxis()->SetTitle(XTitle);
  _TTbar_var_rebin->GetXaxis()->SetTitleSize(0.05);
  _TTbar_var_rebin->GetXaxis()->SetTitleOffset(3.3);
  _TTbar_var_rebin->GetXaxis()->SetTitleFont(42);
  _TTbar_var_rebin->GetXaxis()->SetNdivisions(505);
  _TTbar_var_rebin->GetYaxis()->SetNdivisions(505);
  _TTbar_var_rebin->GetYaxis()->SetLabelFont(42);
  _TTbar_var_rebin->GetYaxis()->SetLabelSize(0.04);
  _TTbar_var_rebin->GetYaxis()->SetLabelOffset(0.01);
  _TTbar_var_rebin->GetYaxis()->SetTitle("Events");
  _TTbar_var_rebin->GetYaxis()->SetTitleOffset(1.5);
  _TTbar_var_rebin->GetYaxis()->SetTitleSize(0.05);
  _TTbar_var_rebin->GetYaxis()->SetTitleFont(42);
  _TTbar_var_rebin->SetTitle("");

  double ymin = 10e-2;
  double ymax = 5 * (_TTbar_var_rebin->GetMaximum());

  _TTbar_var_rebin->Draw("HIST");
  _TTbar_var_rebin->SetFillColor(kOrange - 2);
  _TTbar_var_rebin->SetLineWidth(1);
  _TTbar_var_rebin->SetLineStyle(1);
  _TTbar_var_rebin->SetMaximum(ymax);
  _TTbar_var_rebin->SetMinimum(ymin);

  _Data_var_rebin->Draw("E same");
  _Data_var_rebin->SetMarkerColor(1);
  _Data_var_rebin->SetMarkerStyle(20);
  _Data_var_rebin->SetMarkerSize(1.1);
  _Data_var_rebin->SetLineColor(1);
  _Data_var_rebin->SetLineWidth(3);
  _Data_var_rebin->SetMaximum(ymax);
  _Data_var_rebin->SetMinimum(ymin);

  TLegend *leg = new TLegend(0.2, 0.4, 0.3, 0.75);
  leg = new TLegend(0.22, 0.78, 0.5, 0.88);
  leg->SetTextSize(0.03);
  leg->AddEntry(_Data_var_rebin, "Data (bkg. subtracted)", "p");
  leg->AddEntry(_TTbar_var_rebin, "TTbar", "f");
  leg->Draw("same");

  c1_1->Modified();
  c1->cd();

  TH1D *_SF_pt = (TH1D *)_Data_var_rebin->Clone();
  TH1D *_MC_pt = (TH1D *)_TTbar_var_rebin->Clone();
  ...
  TF1 *Mypol = new TF1("Mypol", "pol8", 0.0, 1000.0);
  _SF_pt->Fit("Mypol", "R");

  TH1D *hint = new TH1D("hint", "", 185, 0.0, 1000.0);
