...
//   TString VAR = "PNetMD_Xbb_Legacy_AN";
  TString XTitle = JET+VAR;
...
//  XTitle = "FatJet1 - T_{Xbb}";
  XTitle = "FatJet1 - X_{bb}";
...
//  TString erA="PostBPix";
  ...
  TFile* f_Data   = new TFile("Data/Data_"+erA+".root");
  TFile* f_MC    = new TFile("MC/Histograms_TTtoLNu2Q_"+erA+".root");
...
//  _QCD_var->Scale(Offline_BTG_SF);
  ...
  double ymin=10e-2;
  double ymax=5*(_QCD_var->GetMaximum());
...
// ******************************* Legend
    ...
    leg->AddEntry(_QCD_var,"TTbar","f");
    leg->AddEntry(ratioErrH,"MC stat. unc.","f");
    leg->Draw("same");

    TPaveText *t1 = new TPaveText(0.05,0.92,0.5,0.97);
    t1->SetFillColor(0);
    t1->SetTextColor(1);
    t1->SetLineWidth(2);
    t1->SetTextFont(22);
    t1->SetTextSize(0.028);
    t1->AddText("                                            CMS (2023 C-D EGamma+Muon) #it{preliminary} ~ 21.7 fb^{-1}");
    t1->Draw("same");
