#!/bin/tcsh
set input = $1
cat includes_qcd.txt  >  Making_Tree_$input.C
echo 'void Making_Tree_'$input'()' >> Making_Tree_$input.C
echo '{' >> Making_Tree_$input.C
cat JEC_JER_PostBPix.txt >> Making_Tree_$input.C
echo ' TFile *f = new TFile("Histograms_'$input'.root","RECREATE");' >> Making_Tree_$input.C
echo '' >> Making_Tree_$input.C
cat histos.txt    >> Making_Tree_$input.C
echo 'TFile *f1 = new TFile("/eos/home-t/tumasyan/HHTo4B/Data_2023/Legacy/PostBPix/'$input'.root");' >> Making_Tree_$input.C
echo '' >> Making_Tree_$input.C
